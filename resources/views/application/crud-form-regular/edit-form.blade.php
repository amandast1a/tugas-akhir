                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form class="needs-validation" action="{{ route('jabatan.regular.update', $form->id) }}" method="POST" enctype="multipart/form-data" novalidate>
                                    ...
                                    @foreach ([
                                        'doc_suratPengantar' => 'SURAT PENGANTAR (dari Kepala SKPD) file max 1MB',
                                        'doc_pangkatTerakhir' => 'SK PANGKAT TERAKHIR',
                                        'doc_jabatanAtasan' => 'SK JABATAN ATASAN LANGSUNG',
                                        'doc_tandaLulus' => 'SURAT TANDA LULUS UJIAN DINAS (Jika Ada)',
                                        'doc_skAlihtugas' => 'SK ALIH TUGAS (Jika Ada)',
                                        'doc_penilaian2022' => 'PENILAIAN KINERJA 2022',
                                        'doc_penilaian2023' => 'PENILAIAN KINERJA 2023',
                                        'doc_skCpns' => 'SK CPNS (KP PERTAMA)',
                                        'doc_skPns' => 'SK PNS (KP PERTAMA)',
                                        'doc_STTPL' => 'STTPL (KP PERTAMA)',
                                        'doc_beritaAcarasumpah' => 'BERITA ACARA SUMPAH (KP PERTAMA)',
                                        'doc_ijazah' => 'IJAZAH *ketika diangkat (KP PERTAMA)',
                                        'doc_transkrip' => 'TRANSKRIP *ketika diangkat (KP PERTAMA)',
                                    ] as $field => $label)
                                        <div class="mb-3">
                                            <label class="form-label" for="{{ $field }}">{{ $label }}</label>
                                            <input type="file" class="form-control" id="{{ $field }}" name="{{ $field }}" accept="application/pdf" />
                                            @if ($form->$field)
                                                <small>Current file: <a href="{{ asset('assets/documentRegular/' . $form->$field) }}" target="_blank">View current file</a></small>
                                            @endif
                                        </div>
                                    @endforeach
                                    <div class="row">
