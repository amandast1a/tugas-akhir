...
<html
  ...
  <head>
    <title>Table formulir usul kenaikan pangkat jabatan struktural - Pengusul</title>
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
                ...
                <div class="card">
                    <div class="col-md-12 d-flex justify-content-between align-items-cente">
                        <h5 class="card-header">Table Formulir usul kenaikan pangkat jabatan struktural</h5>
                        <a href="/proses-table-struktural" class="btn btn-outline-primary" style="height: fit-content; padding: 15px 30px 15px 30px; margin-top: 20px;">Lihat Proses</a>
                    ...
                    <form action="{{ route('jabatan.struktural.search') }}" method="GET">
                    ...
                    <table class="table">
                        ...
                        <tbody class="table-border-bottom-0">
                            ...
                            <tr>
                                ...
                                <td>{{ $item->time }}</td>
                                <td>
                                    <a href="{{ route('jabatan.struktural.show', $item->id) }}" class="btn btn-outline-primary"> Detail data</a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
                    ...
                    {{-- <nav aria-label="Page navigation" style="margin: 30px 30px 30px 20px;">
                        <ul class="pagination pagination-sm">
                            @if ($Form_struktural->currentPage() > 1)
                                <li class="page-item prev">
                                    <a class="page-link" href="{{ $Form_struktural->previousPageUrl() }}">
                                        <i class="tf-icon fs-6 ti ti-chevrons-left"></i>
                                    </a>
                                </li>
                            @endif

                            @if ($Form_struktural->lastPage() > 1)
                                <!-- First Page Link -->
                                @if ($Form_struktural->currentPage() > 3)
                                    <li class="page-item">
                                        <a class="page-link" href="{{ $Form_struktural->url(1) }}">1</a>
                                    </li>
                                    @if ($Form_struktural->currentPage() > 4)
                                        <li class="page-item disabled"><span class="page-link">...</span></li>
                                    @endif
                                @endif

                                <!-- Page Numbers -->
                                @for ($i = max(1, $Form_struktural->currentPage() - 2); $i <= min($Form_struktural->lastPage(), $Form_struktural->currentPage() + 2); $i++)
                                    <li class="page-item {{ $i == $Form_struktural->currentPage() ? 'active' : '' }}">
                                        <a class="page-link" href="{{ $Form_struktural->url($i) }}">{{ $i }}</a>
                                    </li>
                                @endfor

                                <!-- Last Page Link -->
                                @if ($Form_struktural->currentPage() < $Form_struktural->lastPage() - 2)
                                    @if ($Form_struktural->currentPage() < $Form_struktural->lastPage() - 3)
                                        <li class="page-item disabled"><span class="page-link">...</span></li>
                                    @endif
                                    <li class="page-item">
                                        <a class="page-link" href="{{ $Form_struktural->url($Form_struktural->lastPage()) }}">{{ $Form_struktural->lastPage() }}</a>
                                    </li>
                                @endif
                            @endif

                            @if ($Form_struktural->hasMorePages())
                                <li class="page-item next">
                                    <a class="page-link" href="{{ $Form_struktural->nextPageUrl() }}">
